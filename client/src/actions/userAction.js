...
export const resendVerification = (email) => async (dispatch) => {
  try {
    ...
    const { data } = await axios.post("/api/v1/user/verify", email, config);
    ...
    dispatch({
      type: "RESEND_VERIFICATION_LINK_FAIL",
      payload: error.response.data.message,
    });
  }
};
...
export const validateVerification = (token) => async (dispatch) => {
  try {
    dispatch({ type: "VALIDATE_USER_REQUEST" });

    const config = { headers: { "Content-Type": "application/json" } };

    const { data } = await axios.put(`/api/v1/user/verify/${token}`, config);
    dispatch({
